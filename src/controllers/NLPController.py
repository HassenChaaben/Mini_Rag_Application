...
class NLPController(BaseController):
    ...
    async def get_vector_db_collection_info(self , project:Project):
        ...
        # convert any object to json serializable dict
        return json.loads(json.dumps(collection_info, default=lambda x: x.__dict__))

    async def index_into_vector_db(self , project:Project , chunks:List[DataChunk] , do_reset:int=0 , chunks_ids = list[int]):
        ...
        # step 3 : create collection if not exits 
        
        _ = await self.vectordb_client.create_collection(
            collection_name=collection_name,
            embedding_size=self.embedding_client.embedding_size,
            do_reset=do_reset # error : the delete collection done already in create_collection in vectordbprovider PGvectorDBProvider or qdrantdbprovider
        )
    ...
    async def answer_rag_question(self , project:Project , query:str , limit:int =5):
        ...
        # system_prompt = """You are an assistant to generate a response for the user .
        #                    You will be provided by set of documents associated with the user query . 
        #                    You have to generate a response based on the documents provided .Ignore the Document if not relevant to the query . 
        ...
        footer_prompt = self.template_parser.get(group = "rag" , key = "footer_prompt", vars={"user_question": query})

        chat_history = [
            self.generation_client.contruct_prompt(
